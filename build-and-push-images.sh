...
docker push tiffanyjoelle/webserver-prod:$NEW_VERSION
#webserver-prod is whatever name we want, but it has to match the compose image name

docker buildx build --platform linux/amd64  -t tiffanyjoelle/backend-prod:$NEW_VERSION -f backend/Dockerfile ./backend --no-cache
docker push tiffanyjoelle/backend-prod:$NEW_VERSION